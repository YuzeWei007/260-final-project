...
population |>
  ggplot(aes(x = reorder(state_name, population), y = population,fill = as.factor(year))) + 
  geom_col() + 
  coord_flip() +
  facet_wrap(~ year) + 
  labs(x = "State", y = "Population", title = "Population of States in 2020 and 2021")
...
regions <- map_df(regions, function(x)
    ...
    mutate(region_name = ifelse(region_name == "New York and New Jersey, Puerto Rico, Virgin Islands", "NY and NJ, PR, VI", region_name))
print(regions)
...
No, this is not all the data here. Because the CDC has a default limit. We see exactly 1,000 rows. We should be seeing over $52 \times 3$ rows per state.
...
api <- "https://data.cdc.gov/resource/pwn4-m3yp.json?$limit=1000000000"
cases_raw <- request(api) |> 
  req_perform() |> 
  resp_body_json(simplifyVector = TRUE) |> 
  as_tibble()
  
cases <- cases_raw |> mutate(date = as.Date(end_date),cases = as.numeric(new_cases)) |> select(state, date, cases)
...
cases |> 
  ...
  labs(title = "COVID-19 Cases per 100,000 by State and Region (2020-2021)", x = "Date", y = "Cases per 100,000 People") +
  theme_minimal() +
