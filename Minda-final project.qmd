...
results <- deaths |>
  ...
  group_by(period) |> 
  summarise(
    peak_death = max(deaths, na.rm = TRUE),                      # Max deaths
    peak_date_death = date[which.max(deaths)],                   # Date for max deaths
    peak_cases = max(cases, na.rm = TRUE),                       # Max cases
    peak_date_cases = date[which.max(cases)]                     # Date for max cases
  )
